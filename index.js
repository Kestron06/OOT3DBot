...
function searchLog(term){
    ...
    var slog=JSON.parse(fs.readFileSync("latestSpoiler.json"));
    var locs=[];
...
var lastStarted;
client.once("ready",()=>{
    console.log(`Logged OOT3DRBot Functions into ${client.user.tag}`);
            ...
            if(a.name.toLowerCase().endsWith("spoilerlog.xml")){
                fetch(a.url).then(d=>d.text()).then(d=>{
                    parseString(d, (e,result)=>{
                        ...
                        var opts={};
                        for(var i=0;i<result["spoiler-log"].settings[0].setting.length;i++){
                            opts[result["spoiler-log"].settings[0].setting[i]["$"].name]=result["spoiler-log"].settings[0].setting[i]["_"];
                        }
                        msg.reply({
                            ...
                            embeds: [{
                                ...
                                "fields": [
                                    ...
                                    {
                                        "name":`Starting Age`,
                                        "value": opts["Starting Age"],
                                        "inline":true
                                    },
                                    {
                                        "name":`Door of Time`,
                                        "value": opts["Door of Time"],
                                        "inline":true
                                    },
                                    {
                                        "name":`Rainbow Bridge`,
                                        "value": opts["Rainbow Bridge"],
                                        "inline":true
                                    },
                                    {
                                        "name":`Ganon's Boss Key`,
                                        "value": opts["Ganon's Boss Key"],
                                        "inline":true
                                    },
                                    {
                                        "name":`Entrance Shuffle`,
                                        "value": opts["Shuffle Entrances"],
                                        "inline":true
                                    }
                                ]
                            }]
