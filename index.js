      ...
      .then(def=>{
        if(!def.includes("No Definitions Found")){
          ...
          */
          ...
          msg.reply({content:"Definition:",embeds:[embed]});
        }
        else{
		...
		if(comm.startsWith("makepreset")){
			...
			msg.reply({content:"Autogenerated preset file with the same settings as the spoiler log.",files:["./autoGeneratedPreset.xml"]});
		}
		...
		msg.attachments.forEach(a=>{
			if(a.name.includes("spoilerlog.xml")){
				fetch(a.url).then(d=>d.text()).then(d=>{
					...
					msg.reply({content:"Spoiler Log Parsed."+warning+"\n\nTo search item locations before another spoiler is uploaded, type `!hint ITEM`. To access this log later after more have uploaded, reply to the spoiler log you wish to search and type `!hint ITEM`",embeds:[{
			      "type": "rich",
